...
2. A runtime **workaround**: a Docker socket proxy that intercepts `POST /containers/create` and injects the three HostConfig fields the container needs (`CapAdd=[SYS_ADMIN]`, `Devices=[/dev/fuse]`, `SecurityOpt=[apparmor:unconfined]`
